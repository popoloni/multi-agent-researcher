...
describe('ResearchProgress Component - Core Functionality', () => {
  ...
  describe('Stage Indicators', () => {
    ...
    test('highlights active stage correctly', () => {
      ...
      // At 50% progress, some stages should be completed (green) and some should be pending
      const completedStages = container.querySelectorAll('.border-green-200');
      const pendingStages = container.querySelectorAll('.border-gray-200');
      
      // Should have both completed and pending stages
      expect(completedStages.length + pendingStages.length).toBeGreaterThan(0);
    });
  ...
  describe('Error States', () => {
    test('handles null status gracefully', () => {
      ...
      // Use getAllByText to handle multiple "0%" occurrences
  ...
  describe('Performance Metrics Display', () => {
    test('shows default values when no specific data provided', () => {
      const mockStatus = {
        status: 'executing',
      ...
      // Should show default agent count
      expect(screen.getByText('3')).toBeInTheDocument(); // Default agent count
      expect(screen.getByText('Active Agents')).toBeInTheDocument();
    });

    test('calculates efficiency percentage correctly', () => {
      const mockStatus = {
        status: 'executing',
